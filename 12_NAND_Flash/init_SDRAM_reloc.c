...
//Copy .text .rodata .data to SDRAM
void copy2sdram(void)
{
  ...
  if(isBootROMfromNOR())
  {
    ...
    //if(*((volatile unsigned int*) 0x0000006a)==*((volatile unsigned int*) 0x3000006a))
    //GPFDAT &=~(1<<6);  //Debug
  }
    else
    {
      nand_flash_init();
      read_nand_flash((volatile unsigned int)NAND_NOR_src, (volatile unsigned char*)SDRAM_start, ((int)&__bss_start) - ((int)&__code_start));
      //Copy data from addr=0@nand to addr=SDRAM_start@sdram
      //char*, read one byte/address
      //if(*((volatile unsigned int*) 0x0000006a)==*((volatile unsigned int*) 0x3000006a))
      //GPFDAT &=~(1<<5);//Debug
    }
}
...
/*
  Check if addr=0 writeable when boot from NAND or NOR
  Yes = 0, NAND
  NO  = 1, NOR
*/
int isBootROMfromNOR(void)
{
  volatile unsigned int* p=(volatile unsigned int*) 0;
  volatile unsigned int val=*p;
  *p=0xdeadc0de;
  if(*p==0xdeadc0de) //NAND
  {
    *p=val;
    return 0;
  }else
